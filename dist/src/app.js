...
function renderGuidePage() {
  const selectedGuide = state.selectedGuideId ? diseaseGuides.find((guide) => guide.id === state.selectedGuideId) : null;
  if (selectedGuide) return renderDiseaseDetail(selectedGuide);
  ...
  const filtered = source.filter((guide) => {
    const searchText = `${local(guide.name)} ${local(guide.category)} ${local(guide.summary)} ${local(guide.safeFoods)?.join(' ') || ''} ${local(guide.badFoods)?.join(' ') || ''}`.toLowerCase();
    return !query || searchText.includes(query);
  });

  return `<section>
    <h2 class="section-title">📚 ${t('diseaseLibrary')}</h2>
...
function renderDiseaseCard(guide) {
  const isPersonal = (state.profile.diseases || []).includes(guide.id);
  return `<article class="card disease-card col-4 ${isPersonal ? 'personal-match' : ''}">
    <span class="badge">${local(guide.category)}</span>
    ${isPersonal ? `<span class="badge safe-badge">${t('specificForPatient')}</span>` : ''}
    ...
    <div class="mini-list">
      ...
      <span>🚫 ${t('badFoods')}</span>
      <span>🍽️ ${t('recipes')}</span>
    </div>
    <div class="action-row" style="margin-top:14px">
      <button class="btn btn-soft btn-full" data-action="open-guide" data-id="${guide.id}">${t('openDisease')}</button>
    </div>
  </article>`;
}

function renderDiseaseDetail(guide) {
  return `<section>
    <button class="btn btn-ghost" data-action="back-guide">← ${t('backToGuide')}</button>
    <article class="card disease-detail">
      <div class="detail-head">
        <div>
          <span class="badge">${local(guide.category)}</span>
          <h2 class="section-title" style="margin-bottom:8px">${local(guide.name)}</h2>
          <p class="lead small-lead">${local(guide.summary)}</p>
        </div>
      </div>
    ...
    <article class="card food-detail">
      <h3 class="card-title">🥗 ${t('dietSection')}</h3>
      <div class="detail-grid">
        ${guideSection('🟢', t('safeFoods'), local(guide.safeFoods), 'good')}
        ${guideSection('🔴', t('badFoods'), local(guide.badFoods), 'bad')}
        ${guideSection('📌', t('dietRules'), local(guide.diet), '')}
      </div>
    </article>

    <article class="card recipes-card">
      <h3 class="card-title">🍽️ ${t('recipes')}</h3>
      <div class="meal-grid">
        ${mealSection('🌅', t('breakfast'), local(guide.recipes?.breakfast || []))}
        ${mealSection('☀️', t('lunch'), local(guide.recipes?.lunch || []))}
        ${mealSection('🌙', t('dinner'), local(guide.recipes?.dinner || []))}
      </div>
    </article>
  </section>`;
}
...
function guideSection(icon, title, items, className) {
  ...
  return `<div class="guide-row ${className}"><strong>${icon} ${title}</strong><p>${safeItems.map((item) => `• ${escapeHtml(item)}`).join('<br>')}</p></div>`;
}

function mealSection(icon, title, items) {
  const safeItems = Array.isArray(items) ? items : [];
  return `<div class="meal-card"><strong>${icon} ${title}</strong><ul>${safeItems.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`;
}
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-action="go-page"]').forEach((btn) => btn.addEventListener('click', () => setState({ page: btn.dataset.page, selectedGuideId: btn.dataset.page === 'guide' ? state.selectedGuideId : null })));
  app.querySelectorAll('[data-action="set-guide-mode"]').forEach((btn) => btn.addEventListener('click', () => setState({ guideMode: btn.dataset.mode, selectedGuideId: null })));
  app.querySelectorAll('[data-action="open-guide"]').forEach((btn) => btn.addEventListener('click', () => setState({ selectedGuideId: btn.dataset.id })));
  app.querySelectorAll('[data-action="back-guide"]').forEach((btn) => btn.addEventListener('click', () => setState({ selectedGuideId: null })));
